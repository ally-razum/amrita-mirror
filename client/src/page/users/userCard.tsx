...
function UserCard() {
  ...
  useEffect(() => {
    const fetchCardData = async () => {
      try {
        ...
        console.error(
          `Ошибка при получении данных карточки с ID: ${userID}`,
          error,
        );
        setLoading(false);
      }
  ...
  const validateForm = () => {
    ...
    if (
      ...
      !fullNameRegex.test(userData!.userFullName)
    ) {
    ...
    if (!userData!.userLogin || !loginRegex.test(userData!.userLogin)) {
      newErrors.userLogin =
        "Логин должен состоять из одного слова. Допускается ввод латинских букв и цифр";
      isValid = false;
    }
    ...
    >
      <Box marginLeft={5}>
        ...
        >
          ...
          <Button
            sx={{ marginLeft: 2, backgroundColor: "#ab47bc", borderRadius: 20 }}
            variant="contained"
